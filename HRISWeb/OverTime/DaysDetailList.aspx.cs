...
{
    public partial class DaysDetailList : System.Web.UI.Page
    {
        [Dependency]
        ...
        private void BindDayTypes(DropDownList dropDownList)
        {
            List<DayTypesEntity> dayTypesEntities = new List<DayTypesEntity>();
            try
            {
                dayTypesEntities.Add(new DayTypesEntity() { DayTypeCode = -1, DayTypesName = "" });
                dayTypesEntities.AddRange(dayTypesBLL.GetDayTypesListForDropdown(SessionManager.GetSessionValue<DivisionByUserEntity>(SessionKey.WorkingDivision).GeographicDivisionCode, SessionManager.GetSessionValue<DivisionByUserEntity>(SessionKey.WorkingDivision).DivisionCode));
                dropDownList.DataSource = dayTypesEntities;
        ...
        protected void btnAdd_ServerClick(object sender, EventArgs e)
        {
            try
            {
                BindDayTypes(cboDayTypeCode);
                txtDaysDetailCode.Text = "0";
                cboDayTypeCode.SelectedIndex = 0;
                txtDescriptionDay.Text = "";
                dtpCodeDateBase.Text = "";
            }
            catch (Exception ex)
            {
                if (ex is DataAccessException
        ...
        {
            DaysDetailEntity daysDetailEntity = new DaysDetailEntity();
            try
            {
                daysDetailEntity.GeographicDivisionCode = SessionManager.GetSessionValue<DivisionByUserEntity>(SessionKey.WorkingDivision).GeographicDivisionCode;
                daysDetailEntity.DivisionCode = SessionManager.GetSessionValue<DivisionByUserEntity>(SessionKey.WorkingDivision).DivisionCode;
                daysDetailEntity.DaysDetailCode = string.IsNullOrEmpty(txtDaysDetailCode.Text) ? 0 : Convert.ToInt32(txtDaysDetailCode.Text);
                daysDetailEntity.DayTypeCode = string.IsNullOrEmpty(cboDayTypeCode.SelectedValue) ? 0 : Convert.ToInt32(cboDayTypeCode.SelectedValue);
                daysDetailEntity.DescriptionDay = txtDescriptionDay.Text;
                daysDetailEntity.CodeDateBase = string.IsNullOrEmpty(dtpCodeDateBase.Text) ? new DateTime(1, 1, 1) : DateTime.ParseExact(String.Format("{0} 00:00", dtpCodeDateBase.Text), "MM/dd/yyyy HH:mm", CultureInfo.InvariantCulture, DateTimeStyles.None);
                bool isSuccess = false;


                if (daysDetailEntity.DaysDetailCode.Equals(0))
                {
                    daysDetailEntity.CreatedBy = UserHelper.GetCurrentFullUserName;
                    isSuccess = daysDetailBLL.AddDaysDetail(daysDetailEntity);
                }
                else
                {
                    daysDetailEntity.LastModifiedUser = UserHelper.GetCurrentFullUserName;
                    isSuccess = daysDetailBLL.UpdateDaysDetail(daysDetailEntity);
                }
                ...
                {
                    MensajeriaHelper.MostrarMensaje(Page, TipoMensaje.Error, Convert.ToString(GetLocalResourceObject("msgDublicateRecord")));
                }
            }
            catch (Exception ex)
            {
        ...
        protected void btnEdit_ServerClick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    int daysDetailCode = Convert.ToInt32(grvList.DataKeys[Convert.ToInt32(hdfSelectedRowIndex.Value)].Value);
                    DisplayDaysDetailInformation(daysDetailCode);
        ...
        protected void btnDelete_ServerClick(object sender, EventArgs e)
        {
            try
            {
                ...
                var result = daysDetailBLL.DeleteDaysDetail(code);

                if (result)
                {
                    PageHelper<DaysDetailEntity> pageHelper = SearchResults(PagerUtil.GetActivePage(blstPager));
        ...
        /// <summary>
        /// Displays on modal the overTime information
        /// </summary>
        /// <param name="daysDetailCode">The days detail code to search</param>
        private void DisplayDaysDetailInformation(int daysDetailCode)
        {
            try
            {
                DaysDetailEntity daysDetailEntity = daysDetailBLL.GetDaysDetailByDaysDetailCode(daysDetailCode);
                if (daysDetailEntity != null)
                {
                    BindDayTypes(cboDayTypeCode);
                    txtDaysDetailCode.Text = daysDetailEntity.DaysDetailCode.ToString();
                    cboDayTypeCode.SelectedValue = daysDetailEntity.DayTypeCode.ToString();
                    txtDescriptionDay.Text = daysDetailEntity.DescriptionDay.ToString();
                    dtpCodeDateBase.Text = daysDetailEntity.CodeDateBase.ToString("MM/dd/yyyy HH:mm");
                }
                else
                {
        ...
        private PageHelper<DaysDetailEntity> SearchResults(int page)
        {
            ...
            hdfSelectedRowIndex.Value = "-1";
            //
            if (daysDetailBLL  == null)
            {
                daysDetailBLL  = Application.GetContainer().Resolve<IDaysDetailBLL<DaysDetailEntity>>();
            }
            PageHelper<DaysDetailEntity> pageHelper = daysDetailBLL.GetDaysDetailList(
                ...
                , string.IsNullOrEmpty(txtDetailCode.Text) ? 0 : int.Parse(txtDetailCode.Text)
                , cboTypeCode.SelectedValue == "0" ? 0 : int.Parse(cboTypeCode.SelectedValue) 
                , txtDescriptionDayFilter.Text
                , string.IsNullOrEmpty(dtpDateBase.Text) ? (DateTime?)null : DateTime.ParseExact(String.Format("{0} 00:00", dtpDateBase.Text), "MM/dd/yyyy HH:mm", CultureInfo.InvariantCulture, DateTimeStyles.None)
                , sortExpression
                , sortDirection
                , page
                , pageSizeValue);
        ...
        private void DisplayResults(PageHelper<DaysDetailEntity> pageHelper)
        {
