...
namespace DOLE.HRIS.Application.Automation.Selenium
{
    [TestFixture]
    [Category("Propositos")]
    public class TC001CreacionTiposFormacionTest
    {
        ...
        public void SetUp()
        {
            ...
            seleniumEntities.Add(new SeleniumEntity("TP001", "TP001_TpoFormacionCreado"));
            seleniumEntities.Add(new SeleniumEntity("TP002", "TP002_TpoFormacionCreado"));
        ...
        [Test]
        public async Task TC001CreacionTiposFormacion()
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                myCommons.SearchTrainingTypeByCode(wait, driver, js, seleniumEntities[i].Code);
                Thread.Sleep(1000);
                wait.Until(ExpectedConditions.ElementIsVisible(By.Id("dvTypeTrainingCode")));
                myCommons.Log.Information($"{DateTime.Now}: Buscando el Tipo Formación creado  " + seleniumEntities[i].Code);
                ...
                Assert.AreEqual(seleniumEntities[i].Code, displayedCode, "La código del Tipo de formación no coincide.");
                myCommons.Log.Information($"{DateTime.Now}: Validando el Assert por Código de Tipo de Formación  " + seleniumEntities[i].Code);


                string displayedDescription = driver.FindElement(By.Id("dvTypeTrainingName")).Text;
                Assert.AreEqual(seleniumEntities[i].Description, displayedDescription, "La descripción del Tipo de formación no coincide.");
                myCommons.Log.Information($"{DateTime.Now}: Validando el Assert por Descripción del Tipo de formación  " + seleniumEntities[i].Description);
