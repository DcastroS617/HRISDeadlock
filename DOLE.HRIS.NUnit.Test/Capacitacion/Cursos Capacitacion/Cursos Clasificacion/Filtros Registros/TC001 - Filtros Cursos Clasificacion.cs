...
namespace DOLE.HRIS.Application.Automation.Selenium
{
    ...
    [Category("Cursos")]
    public class TC001FiltrosCursosClasificacionTest
    {
        ...
        private readonly SeleniumEntity seleniumEntities = new SeleniumEntity("CCFL001", "CCFL001_ClasificaciónAFiltrar");
        ...
        [Test]
        public async Task TC001FiltrosCursosClasificacion()
        {
            ...
            IWebElement element = wait.Until(ExpectedConditions.ElementToBeClickable(By.Id("dvClassificationCourseCode")));

            // Espera para ver si se actualiza
            wait.Until(ExpectedConditions.ElementIsVisible(By.Id("dvClassificationCourseCode")));
            string displayedCode = driver.FindElement(By.Id("dvClassificationCourseCode")).Text;
            myCommons.Log.Information($"Código del Curso de Clasificación recuperado después de la búsqueda: {seleniumEntities.Code}");


            myCommons.Log.Information($"{DateTime.Now}: Se inicia la validación de los assert luego de filtar por ctl00_cntBody_ClassCodeFilter.");
            Assert.AreEqual(seleniumEntities.Code, displayedCode, "El código del Curso de Clasificación no coincide.");
            myCommons.Log.Information($"{DateTime.Now}: Validando el Assert por dvClassificationCourseCode coincida con {seleniumEntities.Code}.");
            ...
            Assert.AreEqual(seleniumEntities.Description, displayedDescription, "El Nombre del Cursos de Clasificación  no coincide." + seleniumEntities.Description);
            myCommons.Log.Information($"{DateTime.Now}: Validando el Assert por dvClassificationCourseDesEs coincida con: {seleniumEntities.Description} ");
            ...
            wait.Until(ExpectedConditions.ElementIsVisible(By.Id("dvClassificationCourseDesEs")));
            displayedDescription = driver.FindElement(By.Id("dvClassificationCourseDesEs")).Text;
            Assert.AreEqual(seleniumEntities.Description, displayedDescription, "el nombre del Curso de Clasificación no coincide." + seleniumEntities.Description);
            myCommons.Log.Information($"{DateTime.Now}: Validando el Assert por dvClassificationCourseDesEs coincida con: {seleniumEntities.Description} ");
