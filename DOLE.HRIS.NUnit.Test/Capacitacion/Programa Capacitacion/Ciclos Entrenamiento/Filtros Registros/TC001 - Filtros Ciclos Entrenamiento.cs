...
namespace DOLE.HRIS.Application.Automation.Selenium
{
    [TestFixture]
    [Category("Ciclo")]
    public class TC001FiltrosCiclosEntrenamientoTest
    {
        ...
        private readonly SeleniumEntity seleniumEntities = new SeleniumEntity("CCF001", "CCF001_CicloEntrenamientoAFiltrar");
        ...
        [Test]
        public async Task TC001FiltrosCiclosEntrenamiento()
        {
            ...
            myCommons.Log.Information($"Código del Ciclo de Entrenamiento recuperado después de la búsqueda: " + seleniumEntities.Code);


            myCommons.Log.Information($"{DateTime.Now}: Se inicia la validación de los assert luego de filtar por ctl00_cntBody_CycleTrainingCodeFilter.");
            Assert.AreEqual(seleniumEntities.Code, displayedCode, "La código del Ciclo de entrenamiento no coincide.");
            myCommons.Log.Information($"{DateTime.Now}: Validando el Assert por dvCycleTrainingCode coincida con {seleniumEntities.Code}.");
            ...
            Assert.AreEqual(seleniumEntities.Description, displayedDescription, "La descripción del Ciclo del Entrenamiento no coincide." + seleniumEntities.Description);
            myCommons.Log.Information($"{DateTime.Now}: Validando el Assert por dvCycleTrainingName coincida con: " + seleniumEntities.Description);

            myCommons.SearchTrainingCycleByName(wait, driver, js, seleniumEntities.Description);
            myCommons.Log.Information($"{DateTime.Now}: Buscar el registro recién creado" + seleniumEntities.Code + seleniumEntities.Description);


            myCommons.Log.Information($"{DateTime.Now}: Digitando valor a buscar en el filtro ctl00_cntBody_CycleTrainingNameFilter.  {seleniumEntities.Description}");
            ...
            wait.Until(ExpectedConditions.ElementIsVisible(By.Id("dvCycleTrainingName")));
            displayedDescription = driver.FindElement(By.Id("dvCycleTrainingName")).Text;
            Assert.AreEqual(seleniumEntities.Description, displayedDescription, "La descripción del Ciclo de Entrenamiento no coincide." + seleniumEntities.Description);
            myCommons.Log.Information($"{DateTime.Now}: Validando el Assert por dvCycleTrainingName coincida con:" + seleniumEntities.Description);
        }
    }
}