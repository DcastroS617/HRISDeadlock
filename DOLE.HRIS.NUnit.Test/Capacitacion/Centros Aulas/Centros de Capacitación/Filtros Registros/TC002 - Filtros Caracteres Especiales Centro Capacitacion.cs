...
namespace DOLE.HRIS.Application.Automation.Selenium
{
    [TestFixture]
    [Category("Aulas")]
    public class TC002FiltrosCaracteresEspecialesCentroEntrenamientoTest
    {
        ...
        private readonly SeleniumEntity seleniumEntities = new SeleniumEntity(@"Centroscript/>test!@#$%^*()__++{}:<>?<script/>;:,.[L¨P¨P*ñ{09'987:Ñ[*?P>.><<!@#$%^&*((", @"CENscript/>test!@#$%^*()__++{}:<>?<script/>;:,.[L¨P¨P*ñ{09'987:Ñ[*?P>.><<!@#$%^&*((");
        ...
        [Test]
        public async Task TC002FiltrosCaracteresEspecialesCentroEntrenamiento()
        {
            ...
            var displayedCode = wait.Until(driver => driver.FindElement(By.Id("dvTrainingCenterCode"))).Text;
            myCommons.Log.Information($"{DateTime.Now}: Esperando que el elemento sea visible dvTrainingCenterCode" + seleniumEntities.Code);

            //string displayedCode = driver.FindElement(By.Id("dvTrainingCenterCode")).Text;
            Assert.AreEqual("Centroscri", displayedCode, "La código del centro no coincide.");
            myCommons.Log.Information($"{DateTime.Now}: Validando el Assert en el campo de Código  " + displayedCode);


            myCommons.BuscarCentroDescription(wait, driver, js, seleniumEntities.Description);
            ...
            var displayedDescription = wait.Until(driver => driver.FindElement(By.Id("dvTrainingCenterDescription"))).Text;

            //string displayedDescription = driver.FindElement(By.Id("dvTrainingCenterDescription")).Text;
            Assert.AreEqual("CENscripttest@__:script:.LPPñ09987:ÑP.@", displayedDescription, "La descripción del centro no coincide.");
